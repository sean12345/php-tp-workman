...
class NotifyMsgController extends CommonController {
    /**
     * 短信通知处理
     *
     * @return mix
    ...
    public function msgProcess() {
        ...
        $worker = new Worker('text://127.0.0.1:10021');

        $worker->name = 'NotifyMsgWorker';
       ...
       if (!is_dir($_filePath)) mkdir($_filePath, 0777, true);
        ...
        $worker->onWorkerStart = function($worker){
            ...
            {
                $taskStartCount = $taskCount = D('SMS/MsgTask', 'Service')->getNotifyTaskCount();
                while ($taskCount > 0) {
                    try{
                        $msgTaskObj = D('SMS/MsgTask', 'Service');
                        // 领取短信通知任务并处理
                        $res = $msgTaskObj->checkoutNotifyTask($currentGateWay);
                        $taskCount = $msgTaskObj->getNotifyTaskCount();
