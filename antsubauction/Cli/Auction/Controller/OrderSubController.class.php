...
class OrderSubController extends CommonController {
    /**
     * 拍单订阅分发服务
     *
     * @return mix
    ...
    public function subProcess() {
        ...
        $worker = new Worker('text://127.0.0.1:10031');

        $worker->name = 'SubOrderWorker';
       ...
       if (!is_dir($_filePath)) mkdir($_filePath, 0777, true);
        ...
        $worker->onWorkerStart = function($worker){
            ...
            {
                $taskCount = D('Auction/OrderSub', 'Service')->getSubTaskCount();
                while ($taskCount > 0) {
                    try{
                        $taskObj = D('Auction/OrderSub', 'Service');
                        // 领取拍单订阅任务并处理
                        $res = $taskObj->checkoutSubTask();
                        $taskCount = $taskObj->getSubTaskCount();
                        // Db::close();
                        D()->db(10, NULL);
                        D()->db(11, NULL);
                        if (!$taskCount) break;
