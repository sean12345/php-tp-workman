...
class SubscribeKeyService extends BaseService {
    ...
    public function getSubscribeList($keyType = 0, $keyValue = '')
    {
        ...
        switch ($keyType) {
            ...
            case 3:
                if (!empty($keyValue[1])) {
                    $condition['key_name'] = array(array('EQ', $keyValue[0] . $keyValue[1][0]), array('EQ', $keyValue[0]), array('EQ', -1), 'OR');
                } else {
        ...
        $modelObj = D('Auction/Aums/SubscribeKey', 'Model')->db(11, C('DB_AUMS'), true);
        return $modelObj->field('key_id,key_type,uid,dealer_id,sid')->where($condition)->select();
    }
